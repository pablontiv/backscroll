...
- **Superseded correction signals are cleared on re-sync**: `correction_signals` is append-only with `INSERT OR IGNORE`, so a false positive recorded under older detector rules would outlive any detector fix and keep topping the census. `SyncFiles` deletes a file's signals whose `extraction_version` is NULL or below `CurrentExtractionVersion` before re-detecting, so bumping the extraction version is what propagates a detector fix to already-indexed sessions (drained incrementally by the B1 stale-set, ~200 files/run). Signals already at the current version are untouched, so re-syncing an up-to-date file stays a no-op. **Expired sessions get the same treatment through `BackfillDerived`**, which is their only route: their JSONL is gone, so `SyncFiles` never runs for them again. Backfill discovery now also selects expired paths carrying signals below the current version, clears those, and stamps re-derived signals with `CurrentExtractionVersion` rather than the lossy `0` marker — the stamp is what makes it converge, since a path re-derived under today's rules stops being discovered as stale. (`0` still marks lossy *`tool_events`*, where metadata really was reconstructed from serialized text; a correction signal read the same stored prose either way, so it is not lossy.)
