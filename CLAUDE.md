...
- **Content-type classification**: Messages classified as `text`/`code`/`tool` based on message content types during sync. Tool content is indexed in separate `search_items` rows with `content_type='tool'`. Sync writes only to `search_items`; the `session_events` table is now unused (dropped in a future migration).
