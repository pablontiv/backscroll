...
func TestInspectIndexCurrentShapeIsIdempotent(t *testing.T) {
	...
	if plan.From.AppliedVersion != 15 {
		t.Fatalf("applied version = %d, want 15", plan.From.AppliedVersion)
	}
	if len(plan.Steps) != 0 {
...
func TestInspectIndexRecognizesObservedDevelopmentV13Shape(t *testing.T) {
	...
	if plan.From.AppliedVersion != 13 || len(plan.Steps) != 2 || plan.Steps[0].Version != 14 || plan.Steps[1].Version != 15 {
		t.Fatalf("development V13 plan = %+v, want v13 with v14/v15 migration steps", plan)
	}
}
...
func TestInspectIndexRecognizesCanonicalAndExplicitLegacyV13Shapes(t *testing.T) {
	for _, fixture := range []string{"v13.sql", "v13-legacy-alter-built.sql", "v13-legacy-existing-schema-migrations.sql"} {
		t.Run(fixture, func(t *testing.T) {
			...
			if len(plan.Steps) != 2 || plan.Steps[0].Version != 14 || plan.Steps[1].Version != 15 {
				t.Fatalf("V13-compatible shape has pending steps: %+v, want v14/v15 migrations", plan.Steps)
			}
		})
	}
}
