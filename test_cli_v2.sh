...
./backscroll --help | grep -E "^\s+list|^\s+search|^\s+read|^\s+status|^\s+validate|^\s+rebuild|^\s+purge|^\s+config" | wc -l
CMDCOUNT=$(./backscroll --help | grep -E "^\s+list|^\s+search|^\s+read|^\s+status|^\s+validate|^\s+rebuild|^\s+purge|^\s+config" | wc -l)
if [ "$CMDCOUNT" -eq 8 ]; then
  echo "✓ All 8 v2 commands present in root help"
else
  echo "✗ Expected 8 v2 commands, got $CMDCOUNT"
fi
...
# Test 13: search --indexed-only flag
echo ""
echo "Test 13: search --indexed-only flag"
...
# Test 14: Verify no --robot flag on read
echo ""
echo "Test 14: no --robot flag on v2 read"
