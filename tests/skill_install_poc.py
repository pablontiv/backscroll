...
def probe(binary):
    with tempfile.TemporaryDirectory(
        prefix="skill-poc-", dir=ROOT / ".local-evidence"
    ) as tmp:
        base = Path(tmp)
        home = base / "home"
        home.mkdir()
        ...
        env = {
            **os.environ,
            ...
            "BACKSCROLL_CONFIG_DIR": str(config),
            "BACKSCROLL_DATABASE_PATH": str(base / "index.db"),
            "BACKSCROLL_BIN": str(base / "not-installed"),
            "TMPDIR": str(base),
        ...
        observed = [
            {
                "target": str(d.relative_to(home)),
                "link": d.is_symlink(),
                "sentinel_retained": (d / "local-note").exists(),
                "stale": "STALE" in (d / "SKILL.md").read_text(),
            }
        ...
        fields = dict(
            line.split("=", 1) for line in search.stdout.splitlines() if "=" in line
        )
        assert fields.get("result_0_filepath") == str(document), (
        ...
        print(
            json.dumps(
                {
                    "native_hook_exit": native.returncode,
                    "targets": observed,
                    "robot": search.stdout,
                    "search_stderr": search.stderr,
                },
                indent=2,
            )
        )
