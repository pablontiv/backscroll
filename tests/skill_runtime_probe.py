...
def probe():
    ...
    if sandbox is None:
        raise RuntimeError(
            "probe requires macOS sandbox-exec to prohibit network and external writes"
        )
    binaries = {}
    ...
    try:
        ...
        claude = json.loads(
            run("claude", "plugin", "validate", str(home / ".claude/skills"), "--json")
        )
        claude_target = json.loads(
            run("claude", "plugin", "validate", str(fixture.skill.parent), "--json")
        )
        ...
        with log.open("w") as stderr:
            proc = subprocess.Popen(
                prefix + [binaries["codex"], "app-server", "--stdio"],
                cwd=cwd,
                env=env,
                text=True,
        ...
        for result in (opencode, opencode_only):
            ...
            if len(skills) != 1 or skills[0]["location"] != str(
                fixture.targets[2] / "SKILL.md"
            ):
        ...
        skills = [
            entry
            for group in codex["data"]
            for entry in group["skills"]
            if entry["name"] == "backscroll"
        ]
        if (
            len(skills) != 1
            or skills[0]["path"] != str(fixture.skill / "SKILL.md")
            or not skills[0]["enabled"]
        ):
        ...
        print(
            json.dumps(
                {
                    "versions": versions,
                    "claude_validation": claude,
                    "claude_target_validation": claude_target,
                    "opencode_all_destinations": opencode,
                    "opencode_only": opencode_only,
                    "codex_skills": codex,
                    "codex_stderr": log.read_text(),
                },
                indent=2,
            )
        )
